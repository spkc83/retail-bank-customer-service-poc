...
- a learned CPU domain/intent router;
- native Qwen tool schemas;
- authorization checks for synthetic writes;
- per-user and per-browser isolated SQLite state;
...
exercise the 9B model's tool selection, policy validation, session-isolated
SQLite reads and writes, exact monetary grounding, model-authored response,
multi-turn context, OOD refusal, and credential guard for both demo users.
